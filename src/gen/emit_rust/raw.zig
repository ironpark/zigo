...
pub fn renderRaw(allocator: std.mem.Allocator, writer: *std.Io.Writer, program: abi.Program, options: emit.Options) !void {
    ...
    const value = plugin_hooks.context(allocator, program, options);
    try writer.writeAll(
    ...
    try plugin_hooks.visitFileBegin(options, value, writer);
    ...
    try plugin_hooks.visitFileEnd(options, value, writer);
}
