...
pub fn renderLib(allocator: std.mem.Allocator, writer: *std.Io.Writer, program: abi.Program, options: emit.Options) !void {
    ...
    try writer.writeAll("\npub mod raw;\n");
    if (program.enums.len != 0) try writer.writeAll("\nmod r#enum;\npub use r#enum::*;\n");
    ...
    // The modules a plugin's `rust` slot adds, declared and re-exported here
    // so a plugin never writes a `mod` line of its own and never picks a path
    // `cargo` cannot find.
    try plugin_hooks.writeModuleDeclarations(allocator, writer, program, options);
    ...
    for (program.functions) |function| {
        ...
        const shape_name = shape.raw_name;
        try plugin_hooks.visitFunction(
            options,
            plugin_hooks.methodContext(allocator, program, options, try plugin_hooks.methodFor(allocator, program, options, value, function, shape_name, null)),
            writer,
            function,
        );
    }
...
fn renderFunction(
    ...
    // A claimed declaration keeps its whole generated body under a
    // crate-visible name the claiming plugin's wrapper calls, so nothing
    // public is written for it twice.
    const claim = try plugin_hooks.claimed(options, plugin_hooks.context(allocator, program, options), function);
    if (claim) {
        const checked = try plugin_hooks.checkedNameAlloc(allocator, shape.raw_name);
        defer allocator.free(checked);
        try writer.print("pub(crate) fn {s}(", .{checked});
    } else try writer.print("pub fn {s}(", .{shape.raw_name});
...
pub fn writeBody(
    ...
    if (shape.payload) |payload| {
        ...
        try writer.writeAll("    ");
        if (shape.direct) |direct| if (direct.is_enum) {
            try writer.print("{s}::try_from(", .{direct.public});
            try writeCall(writer, shape, self_expression);
            try writer.print(")\n        .expect(\"zigo: {s}: invalid enum tag for {s}\")\n", .{ function.origin.name, direct.public });
            return;
        };
        try writeCall(writer, shape, self_expression);
    ...
    if (shape.payload) |payload| {
        switch (payload) {
            ...
            .scalar => |scalar| {
                ...
                if (scalar.is_enum) {
                    const expression = try std.fmt.allocPrint(allocator, "{s}::try_from(result){s}.expect(\"zigo: {s}: invalid enum tag for {s}\")", .{ scalar.public, if (shape.declares_errors) "\n        " else "", function.origin.name, scalar.public });
                    defer allocator.free(expression);
                    return writeResultExpression(writer, shape, expression);
                }
            },
...
fn writeCall(writer: *std.Io.Writer, shape: raw.Shape, self_expression: ?[]const u8) !void {
    const wrap = shape.requiresUnsafeCall();
...
pub fn renderErrors(allocator: std.mem.Allocator, writer: *std.Io.Writer, program: abi.Program, options: emit.Options) !void {
    const value = plugin_hooks.context(allocator, program, options);
    try writer.writeAll(
