...
test "an interface file lists the shared methods, Must variants, io.Closer and one assertion per type" {
    ...
    // The builder keeps what it is handed, so the render allocator is the run
    // arena here too.
    try public.renderPublicFile(arena.allocator(), &output.writer, program, options, renderInterfacesBody);
