...
fn renderPublicBody(allocator: std.mem.Allocator, writer: *std.Io.Writer, program: abi.Program, options: emit.Options) !void {
    ...
    for (program.functions) |function| {
        ...
        const replaced = try plugin_hooks.methodReplaced(options, plugin_hooks.context(allocator, program, options), function);
        ...
        try writer.writeAll("}\n");
        try plugin_hooks.runMethodHooks(options, plugin_hooks.methodContext(allocator, program, options, .{
            .public_name = go_name,
...
pub fn writePublicFileBody(allocator: std.mem.Allocator, writer: *std.Io.Writer, program: abi.Program, options: emit.Options, contents: []const u8) !void {
    ...
    if (production_hooks) try plugin_hooks.runFileHooks(options, context, &body.writer, .begin);
    try body.writer.writeAll(contents);
    if (production_hooks) try plugin_hooks.runFileHooks(options, context, &body.writer, .end);
