...
/// Whether any publicly emitted function returns a `[]T` of this struct,
/// which is what both the slice view and the raw-slice conversion need.
pub fn programReturnsStructSlice(program: abi.Program, name: []const u8) bool {
    for (program.functions) |function| {
        if (!emitsPublicFunction(program, function)) continue;
        const node = optionalPayload(function.origin.@"return".errorPayload());
...
/// Whether any publicly emitted function takes a `[]T` of this struct in the
/// given direction: inbound slices need the to-raw conversion, output slices
/// the copy back from raw.
pub fn programUsesStructSlice(program: abi.Program, name: []const u8, out: bool) bool {
    for (program.functions) |function| {
        if (!emitsPublicFunction(program, function)) continue;
        for (function.origin.params) |parameter| {
            if ((parameter.direction == .out) != out or !isValueStructSlice(parameter.type)) continue;
...
fn structConversionReaches(program: abi.Program, root: []const u8, target: []const u8, depth: usize) bool {
    ...
    for (record.fields) |field| {
        if (field.node != .value_struct or common.isPackedValue(program, field.node)) continue;
...
fn parameterStartsStructToRaw(program: abi.Program, parameter: semantic.Parameter) ?[]const u8 {
    ...
    if (node == .value_struct) {
        if (common.isPackedValue(program, node) or common.isTaggedUnionValue(program, node)) return null;
...
pub fn programUsesStructToRaw(program: abi.Program, name: []const u8) bool {
    for (program.functions) |function| {
        if (!emitsPublicFunction(program, function)) continue;
        for (function.origin.params) |parameter| {
...
pub fn programUsesStructFromRaw(program: abi.Program, name: []const u8) bool {
    for (program.functions) |function| {
        if (!emitsPublicFunction(program, function)) continue;
        const returned = optionalPayload(function.origin.@"return".errorPayload());
        if (returned == .value_struct and !common.isPackedValue(program, returned) and
