...
fn framedPluginFile(comptime plugin_index: usize, comptime file: plugin.SourceFile) Emitter {
    return .{
        ...
        .enabled = struct {
            fn enabled(allocator: std.mem.Allocator, program: abi.Program, options: Options) anyerror!bool {
                ...
                if (file.enabled) |predicate| {
                    ...
                    file_options.file = .{ .path = "", .owner = registry.plugins[plugin_index].name, .kind = .plugin, .source_file = file };
...
fn renderLifecycle(_: std.mem.Allocator, writer: *std.Io.Writer, _: abi.Program, options: Options) !void {
    ...
    try writer.writeAll(
        "// Code generated by zigo. DO NOT EDIT.\n\n" ++
            ...
            ")\n\n" ++
            // A handle is any registered pointer type. The packages keep
            // their lifecycle methods unexported and hand them over as
            // method values at init, so nothing about a handle's lifecycle
            // is part of its exported API.
            ...
            "\tentry := handleMethods{\n" ++
            "\t\tacquire: func(handle Handle, operation string) (unsafe.Pointer, error) { return methods.Acquire(handle.(H), operation) },\n" ++
            ...
            "\t\tentry.acquireChild = func(handle Handle, operation string) (unsafe.Pointer, ChildHandle, error) { return methods.AcquireChild(handle.(H), operation) }\n" ++
            ...
            "func CheckedPointer(operation string, value Handle) (unsafe.Pointer, error) { return Acquire(operation, value) }\n" ++
            ...
            "\tif cause, ok := err.(*NativePanicError); ok { for _, handle := range handles { Poison(handle, cause) } }\n" ++
            "\treturn err\n}\n\n" ++
...
test "shared lifecycle is opt-in and contains cross-package identities" {
    ...
    // Handles register their unexported methods; nothing exported is asked
    // of a handle type.
    try std.testing.expect(std.mem.indexOf(u8, source, "type Handle = any") != null);
    try std.testing.expect(std.mem.indexOf(u8, source, "func Register[H any](methods Methods[H])") != null);
    try std.testing.expect(std.mem.indexOf(u8, source, "func PoisonAfterPanic") != null);
    try std.testing.expect(std.mem.indexOf(u8, source, "Zigo") == null);
