...
pub fn renderPublicErrors(allocator: std.mem.Allocator, writer: *std.Io.Writer, program: abi.Program, options: emit.Options) !void {
    return @import("public.zig").renderPublicFile(allocator, writer, program, options, renderErrorsBody);
}

fn renderErrorsBody(allocator: std.mem.Allocator, writer: *std.Io.Writer, program: abi.Program, options: emit.Options) !void {
    try renderErrors(allocator, writer, program, options);
...
fn renderErrors(allocator: std.mem.Allocator, writer: *std.Io.Writer, program: abi.Program, options: emit.Options) !void {
...
pub fn renderPublicHelpers(writer: *std.Io.Writer, program: abi.Program, options: emit.Options) !void {
    ...
    if (has_callbacks) {
        if (options.backend == .purego) {
            ...
            for (program.functions) |function| {
                for (function.origin.params, 0..) |parameter, parameter_index| {
                    ...
                    if (public_writers.callbackNeedsAdapter(program, parameter.type.callback))
                        try public_writers.writeCallbackAdapter(scope, writer, parameter.type.callback, "value")
                    else {
                        try writer.writeByte('(');
            ...
            try writer.writeAll("func zigoDeleteCallbackHandle(handle zigoCallbackHandle) { ");
            try public_writers.writeRawReferencePrefix(writer, options);
            try writer.writeAll("DeleteCallbackHandle(handle) }\n\n");
            if (common.programUsesCallbackDiagnostics(program)) {
                try writer.writeAll("func zigoActiveCallbackHandleCount() int64 { return ");
                try public_writers.writeRawReferencePrefix(writer, options);
                try writer.writeAll("ActiveCallbackHandleCount() }\n");
                try writer.writeAll("func zigoCallbackDispatcherCount() int { return ");
                try public_writers.writeRawReferencePrefix(writer, options);
        ...
        try writer.writeAll("var zigoActiveCallbackHandles atomic.Int64\n\n");
        ...
        if (common.programUsesCallbackDiagnostics(program))
            try writer.writeAll("func zigoActiveCallbackHandleCount() int64 { return zigoActiveCallbackHandles.Load() }\n\n");
        if (common.programHasCallbackCancellation(program)) {
            try writer.writeAll("func setCallbackCancel(handle zigoCallbackHandle, flag *uint32) { ");
            try public_writers.writeRawReferencePrefix(writer, options);
            try writer.writeAll("SetCallbackCancel(handle, flag) }\n\n");
...
fn writeStreamHandleConstructors(writer: *std.Io.Writer, program: abi.Program, options: emit.Options) !void {
    for ([_]semantic.StreamDirection{ .writer, .reader }) |direction| {
        ...
        try writer.print("func zigoNew{s}StreamHandle(value {s}) zigoCallbackHandle {{\n", .{ name, go_type });
        if (options.backend == .purego) {
