...
fn renderGoHandles(allocator: std.mem.Allocator, writer: *std.Io.Writer, program: abi.Program, options: Options) !void {
    for (program.types) |declaration| {
        ...
        if (dependent_parent) |parent_type| {
            const parent_pointer = try std.fmt.allocPrint(allocator, "*{s}", .{parent_type});
        ...
        if (has_dependent_children) try writer.print(
            "// zigoAcquireChild reserves one dependent child atomically with the call pin.\n" ++
                "func ({0s} *{1s}) zigoAcquireChild(operation string) (unsafe.Pointer, error) {{\n" ++
                "\tif {0s} == nil {{\n\t\treturn nil, &HandleError{{Operation: operation}}\n\t}}\n" ++
                "\t{0s}.mu.Lock()\n\tdefer {0s}.mu.Unlock()\n" ++
                "\tif {0s}.closed || {0s}.ptr == nil {{\n\t\treturn nil, &HandleError{{Operation: operation}}\n\t}}\n" ++
                "\tif {0s}.poison != nil {{\n\t\treturn nil, {0s}.poison.poisoned(operation)\n\t}}\n" ++
        ...
        if (constructor) |owned| {
            ...
            if (dependent_parent) |parent_type| {
                const parent_pointer = try std.fmt.allocPrint(allocator, "*{s}", .{parent_type});
                defer allocator.free(parent_pointer);
                try writeStructField(writer, "parent", state_width, parent_pointer);
            }
            ...
            if (dependent_parent) |parent_type| try writer.print(", parent *{s}", .{parent_type});
            ...
            if (dependent_parent != null) try writer.writeAll("\tif state.parent != nil {\n\t\tstate.parent.zigoDropChild()\n\t}\n");
            try writer.writeAll("}\n\n");
...
fn renderHandleChecks(
    ...
    if (function.receiver) |receiver| {
        ...
        if (function.childOfReceiver())
            try writer.print("\tptr, err := {s}.zigoAcquireChild(\"{s} receiver\")\n", .{ receiver_name, operation })
        else
...
fn writeOwnedHandleResult(
    ...
    if (function.childOfReceiver()) {
        const receiver = try receiverVariableAlloc(allocator, function.receiver.?, go_names);
        defer allocator.free(receiver);
        try writer.print(", {s}", .{receiver});
    }
