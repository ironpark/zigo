...
fn renderPublicErrors(allocator: std.mem.Allocator, writer: *std.Io.Writer, program: abi.Program, options: Options) !void {
    ...
    const needs_strconv = program.error_codes.len != 0;
    const raw_import = needs_strconv and !options.raw_colocated;
    const import_count = @as(usize, @intFromBool(has_handle_errors)) +
        @as(usize, @intFromBool(needs_strconv)) + @as(usize, @intFromBool(raw_import));
    if (import_count == 1) {
        try writer.writeAll("\nimport ");
        if (has_handle_errors) try writer.writeAll("\"errors\"\n");
        if (needs_strconv) try writer.writeAll("\"strconv\"\n");
        if (raw_import) {
            try writeRawImport(writer, options, "");
            try writer.writeByte('\n');
