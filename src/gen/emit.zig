...
fn renderRaw(allocator: std.mem.Allocator, writer: *std.Io.Writer, program: abi.Program, options: Options) !void {
    ...
    if (options.ldflags_override) |flags|
        try writer.writeAll(flags)
    else if (options.link_mode == .static) {
        // Naming the archive keeps a same-named shared library in the install
        // directory from satisfying the link instead.
        const stem = try libraryStemAlloc(allocator, package, options);
...
fn libraryStemAlloc(allocator: std.mem.Allocator, package: []const u8, options: Options) ![]u8 {
...
fn renderPuregoRaw(allocator: std.mem.Allocator, writer: *std.Io.Writer, program: abi.Program, options: Options) !void {
    const package_name = try naming.snakeAlloc(allocator, program.package);
    defer allocator.free(package_name);
    const library_stem = try libraryStemAlloc(allocator, package_name, options);
