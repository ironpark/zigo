//! Go's answers to `Target`, plus the Go-only naming rules its emitter uses
//! directly. Everything here is true of Go rather than of the bound Zig
//! library; nothing here describes the C ABI, which every target shares.
const std = @import("std");
const naming = @import("naming");
const semantic = @import("semantic");
const target_api = @import("../targets.zig");
const words = @import("go_words.zig");
...
pub const target: target_api.Target = .{
    ...
    .formatter = .{
        ...
        .leading_args = &.{"-w"},
        .override_flag = "--gofmt <path>",
        .install_hint = "install the Go distribution",
    },
    .vtable = &.{
        .isKeyword = isKeyword,
...
fn vtExportedNameAlloc(allocator: std.mem.Allocator, input: []const u8) anyerror![]u8 {
    return naming.pascalAlloc(allocator, input);
}

fn vtUnexportedNameAlloc(allocator: std.mem.Allocator, input: []const u8) anyerror![]u8 {
    return naming.camelAlloc(allocator, input);
}
...
/// that `build.zig` can reach them too. They are re-exported here so a reader
/// of the Go target finds every rule in one namespace.
...
pub fn paramNamesAlloc(allocator: std.mem.Allocator, zig_names: []const []const u8) ![][]u8 {
    ...
    for (zig_names) |zig_name| {
        var candidate = try naming.camelAlloc(allocator, zig_name);
        if (isKeyword(candidate) or isReservedLocal(candidate)) {
...
pub fn variantTypeNameAlloc(
    ...
    const variant = try naming.pascalAlloc(allocator, variant_name);
    ...
    if (!naming.containsName(taken, candidate)) return candidate;
    {
        const previous = candidate;
        defer allocator.free(previous);
    ...
    var suffix: usize = 2;
    while (naming.containsName(taken, candidate)) : (suffix += 1) {
        const previous = candidate;
        defer allocator.free(previous);
...
pub fn unionFileStemAlloc(
    ...
    const base = try naming.snakeAlloc(allocator, union_name);
    errdefer allocator.free(base);
    if (base.len == 0) return error.InvalidName;
    if (!naming.containsName(taken, base)) return base;
...
test "Go identifier and keyword checks cover boundaries" {
    for ([_][]const u8{ "raw", "native_api", "_private", "x9" }) |name| try std.testing.expect(isIdentifier(name));
    for ([_][]const u8{ "", "_", "type", "9raw", "raw-name" }) |name| try std.testing.expect(!isIdentifier(name));
    for ([_][]const u8{ "type", "range", "go" }) |name| try std.testing.expect(isKeyword(name));
