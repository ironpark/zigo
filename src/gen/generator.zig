...
test "public Go filename uses the normalized package name" {
    ...
    try temporary.dir.access(std.testing.io, "http_client/http_client_type_gen.go", .{});
    ...
    const public_types = try temporary.dir.readFileAlloc(std.testing.io, "http_client/http_client_type_gen.go", std.testing.allocator, .limited(1024));
...
fn expectAllocationFailureLeavesOutputUntouched(allocator: std.mem.Allocator) !void {
    ...
    const existing = [_]struct { path: []const u8, content: []const u8 }{
        ...
        .{ .path = "atomic/atomic_type_gen.go", .content = "old public types" },
...
test "generated errors lock produces an identical second generation" {
    ...
    const paths = [_][]const u8{
        ...
        "repeatable/repeatable_type_gen.go",
