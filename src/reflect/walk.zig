...
pub fn reflect(
    ...
    for (pairings.items) |claim| {
        ...
        if (pairing.findPairing(pairings.items, .destroys, claim.type).? != claim.index)
            return pairingIssue(allocator, "two functions declare `.destroys = \"{s}\"`", .{claim.type});
        if (pairing.findPairing(pairings.items, .constructs, claim.type) == null)
    ...
    for (functions.items, 0..) |*function, index| {
        ...
        if (pairing.findPairing(pairings.items, .constructs, type_name) != null) continue;
        for (functions.items, 0..) |destructor, destructor_index| {
            ...
            try pairing.pair(allocator, &constructors, functions.items, index, destructor_index, type_name, null);
            break;
        }
    }
...
const callbackLayout = zigo.callback_layout.describe;
