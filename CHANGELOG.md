...
생성된 Go 코드는 바뀌지 않습니다. `semantic.json`에는 `.iterator`/`.implements`가 `ext.ITERATOR`/
`ext.IMPLEMENTS`로도 실리고, 등록 타입에 소스 위치 `source`가 추가됩니다.

| 이전 | 이후 |
|---|---|
| `Context.functionOptions(P, function)`, `Context.typeOptions(P, declaration)`, `plugin.readOptions(P, .type, allocator, ext)` | 모든 context의 `optionsOf(P, .function \| .type, ext)`. `readOptions`는 비공개 |
| `Context.options: plugin.Options` (emitter 필드 40여 개) | `Context.options: plugin.PluginOptions` (target, 패키지 경로, `active_package`, configurations, `emitsHelper`, `file`). emitter 옵션은 `src/gen/emit/options.zig` |
| `plugin.Referenced`, `Options.helpers: ?*const Referenced` | emitter의 `references.Referenced`; 플러그인은 `options.emitsHelper(name)`만 |
| `Options.plugins` (골든 케이스용 선택) | generator `Options.plugins`로 이동. 계약에서 삭제 |
| `context.options.facts.get(...)` | `context.facts.get(...)` (`Context.facts`는 읽기 전용, `AnalyzeContext.facts`/`ValidateContext.facts`가 씀) |
| `.site = .{ .path = "semantic.json", .declaration = name }` | `plugin.site.typeSite(declaration)`, `typeSiteFor(declaration, name)`, `documentSite(name)` |
| `naming.pascalAlloc`, 손으로 쓴 `func (v T) Name(...) R {` | `Context.identifierAlloc(allocator, name, .pascal \| .camel)`, `writeFuncHeader`, `writeMethodHeader`, `writeStringLiteral` (`Writers` vtable 확장) |
| `Entry.use(comptime P: anytype, ...)` | `Entry.use(comptime P: zigo.Plugin, ...)`. `zigo.features.iterator`/`implements`는 `zigo.Plugin` 값 |
...
- `zigo.session(.{ .name, .primary, .children, .doc })`로 부모 핸들과 그가 내준 자식 핸들을
  Go 타입 하나로 묶습니다. `New<Name>`은 primary를 받고, 자식은 타입마다 생성되는
  `Add<Type>`이 입양합니다. 한 타입의 자식을 여러 개 입양할 수 있어 primary가 몇 번
  내주든 한 session에 담깁니다. `Add<Type>`은 가변 인자를 받고 session을 돌려주어 호출이
  이어지며 `nil` 핸들은 무시합니다. primary 접근자는 타입 이름, 자식 접근자는 복수형이고
...
- session 계약 위반을 `ZIGO062`로 보고합니다. 자식 없는 선언, 중복, primary를 자식으로
  ...
  받아들입니다. 세션 이름, `New<Name>`, 접근자와 입양 메서드의 충돌은 `ZIGO024`가 잡으며,
  functional options가 만드는 이름과의 충돌도 함께 봅니다.
...
- 플러그인 옵션의 명시적 `null`을 직렬화에서 보존합니다. nullable 옵션에 기본값이 있어도
  전달한 `null`이 기본값으로 바뀌지 않고, 필수 nullable 옵션도 정상적으로 읽힙니다.
- JSON 플러그인이 codepoint 필드의 공개 Go 타입인 `rune`을 유지합니다. JSON용 임시 struct가
  `uint32`를 사용해 marshal·unmarshal 코드가 컴파일되지 않던 문제를 고쳤습니다.
- 선택하지 않은 외부 플러그인은 옵션 검증과 자체 검증도 실행하지 않습니다. 검증과 코드
  생성이 같은 선택 조건을 사용하며, 내장 기능의 검증은 항상 유지됩니다.
...
- C 키워드나 `<stdint.h>`·`<stddef.h>` typedef 이름(`double`, `int`, `uint8_t` 등)을 파라미터
  이름으로 쓰면 `ZIGO021`로 거부합니다. 전에는 그대로 헤더에 나가 `uint8_t double`처럼 C
  컴파일이 깨졌습니다. 헤더는 공개 ABI의 일부이므로 맹글링하지 않고 이름을 바꾸도록 안내합니다.
...
- 의존 module 소스를 훑는 이름 보강이 큰 module 그래프에서 끝나지 않던 문제를 고쳤습니다.
  이미 읽은 파일 판정이 import에 적힌 철자를 그대로 비교해서, 같은 파일이라도
  `a/../b/x.zig`와 `b/x.zig`가 서로 다른 파일로 취급됐습니다. 디렉터리를 넘나드는 import가
  있는 그래프에서는 경로마다 하위 그래프를 다시 훑었고, import가 순환하면 경로가 계속 길어져
  `zigo-reflect`가 CPU를 태우며 끝나지 않았습니다. 이제 판정과 기록 모두 `.`·`..`을 해소한
  경로를 쓰고, 조회는 해시 테이블입니다.
- 이름 보강이 더 채울 것이 없으면 남은 소스를 읽지 않고 멈춥니다. 보강은 비어 있는 값만
  채우므로 doc·소스 위치·파라미터 이름이 모두 찬 뒤의 파일은 문서를 바꿀 수 없습니다.
  수천 개 파일을 가진 의존 그래프에서 파싱 비용이 사라집니다.
