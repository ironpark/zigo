...
추가합니다. 일반 바인딩 작성과 분리된 확장 지점이며, 현재 계약 version은 7.0입니다.
...
- Zig 소스와 C 심볼을 네이티브 쪽에 추가(`native`)

한 패키지에서만 필요한 작은 도우미는 생성 패키지의 사용자 `.go` 파일에 직접 작성하는 편이
단순합니다. 바인딩 함수의 C shim이나 raw ABI를 임의로 바꾸는 플러그인 hook은 제공하지
않습니다. 플러그인이 네이티브 쪽에 할 수 있는 일은 자기 심볼을 **더하는** 것뿐입니다.
