...
| [11-io-streams](../examples/11-io-streams) | `*std.Io.Writer`/`*std.Io.Reader` 파라미터가 `io.Writer`/`io.Reader`로, 버퍼 크기와 경계 횡단 횟수, writer error·panic·reader EOF 경로, `.implements`로 handle이 `io.Writer`·`io.Reader` 구현, `satisfies` 플러그인의 인터페이스 어서션, cgo·purego 병행 |
