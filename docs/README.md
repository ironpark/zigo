...
0.17.x 작성 API에서 옮기는 중이라면 [작성 API 마이그레이션](migration-authoring.md)을 참고하세요.
0.14 → 0.15의 중간 변환 기록은 [이전 마이그레이션 문서](migration-0.15.md)에 있습니다.
...
`.declarations = ...`는 기존 `zigo.define`에 넣는 부분 선언이며,
