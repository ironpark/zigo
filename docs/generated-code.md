...
`go-check`는 커밋된 Go 생성물과 `zigo/` 메타데이터를 현재 선언의 생성 결과와 비교합니다. 필요한 파일이 없거나, 내용이
...
`go-check`는 Go 생성 파일뿐 아니라 `zigo/semantic.json`과 `zigo/errors.lock.json`도 비교합니다.
...
명령 이름을 찾을 때 사용하는 참조 표입니다. 일상 개발에는 `go`, 기본 CI에는
`go-check go-lib`와 별도의 Go 테스트면 충분합니다.
