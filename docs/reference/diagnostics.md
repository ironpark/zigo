...
플러그인 진단 code는 플러그인 이름에 번호를 붙인 형태입니다. 두 번호는 core가 모든
플러그인에 대해 같은 뜻으로 씁니다.

| code | 뜻 |
|---|---|
| `<NAME>001` | 그 플러그인의 옵션 또는 빌드 설정을 읽을 수 없습니다. 옵션 타입과 맞는 값을 쓰거나, `use`로 선언에서 붙이세요. |
| `<NAME>002` | 참조 타입 옵션(`plugin.ref.*`)이 가리키는 선언이 없습니다. 메시지의 path를 이 바인딩이 등록한 선언으로 바꾸세요. |
