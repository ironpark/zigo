...
`pub const keyFromASCII = Key.fromASCII;`처럼 다른 선언을 재노출한 alias를 바인딩하면 doc과
파라미터 이름은 alias가 가리키는 선언에서 읽습니다. alias 자체에 `///` 문서가 있으면 그 문서가
우선합니다. 우변은 식별자 또는 `a.b.c` 꼴의 필드 접근만 따라가며, `@import(...)`·호출·리터럴은
