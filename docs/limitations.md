...
배열, 중첩 slice, extern struct, `[]byte`, optional scalar·string·노드까지 지원합니다
([값 타입](bindings-types.md#materialized-결과-트리)).
...
| `?T` | 파라미터·반환·error payload 지원; materialized 필드는 scalar·string·extern struct·노드 지원; extern struct 필드·콜백·`[]?T`·`??T`는 미지원 |
