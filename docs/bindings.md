...
| 하고 싶은 일 | 관련 절 |
|---|---|
| 노출할 함수 선택, 이름과 하위 패키지 지정 | [기본 구조](#기본-구조), [자동 발견](#명시-목록과-자동-발견), [하위 패키지](#공개-go-하위-패키지) |
| 객체 생성과 수명 관리 | [Opaque handle](#opaque-handle), [호출자 소유 slice](#호출자-소유-slice-반환) |
| 값·구조체·중첩 결과 반환 | [타입 등록](#타입-등록-선택), [Extern struct](#extern-struct-값), [Materialized 결과](#materialized-결과-트리) |
| Go 콜백, 스트림, 취소 연결 | [함수 메타데이터](#함수-메타데이터), [스트림 파라미터](#stdio-스트림-파라미터), [취소](#취소-cancel) |
| 여러 handle을 같은 Go 인터페이스로 사용 | [인터페이스](#인터페이스) |
| 오류 처리와 GoDoc 확인 | [Go error 판별](#생성된-go-error-판별), [Doc 주석](#doc-주석) |
