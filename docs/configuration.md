...
의존 module 쪽에서 읽거나 파싱하지 못한 파일은 경고로 남기고 건너뜁니다. 조건부 import나
생성 module처럼 그 빌드 구성에 없는 파일이 있어도 생성은 계속됩니다. 바인딩 자신의 파일과
root module은 지금처럼 오류입니다. 도달한 root 바깥의 파일은 이름과 doc만 가져오고 소스
위치는 기록하지 않습니다.
