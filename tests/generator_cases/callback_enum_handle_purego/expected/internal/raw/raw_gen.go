...
var DefaultLibraryName = map[string]string{"darwin": "libcbenum_zigo.dylib", "linux": "libcbenum_zigo.so", "windows": "cbenum_zigo.dll"}[runtime.GOOS]
...
type nativeBindings struct {
	...
	fnOnLocation func(uintptr, uintptr)
	fnOnStream func(uintptr, uintptr)
	fnOnView func(uintptr, uintptr)
}
...
var callbackPointers [3]uintptr
...
func ensureCallbackDispatchers() {
	callbackDispatchersOnce.Do(func() {
		...
		callbackPointers[1] = purego.NewCallback(func(p0 unsafe.Pointer, p1 int32, p2 uint) (result uintptr) {
			entry, stored, ok := acquireCallback(uintptr(p2))
			if !ok { return 0 }
			defer releaseCallback(entry)
			defer func() { if value := recover(); value != nil { entry.record(value); result = 0 } }()
			callback := stored.(func(unsafe.Pointer, int32))
			callback(p0, p1)
			return 0
		})
		callbackPointers[2] = purego.NewCallback(func(p0 unsafe.Pointer, p1 uint) (result uintptr) {
			entry, stored, ok := acquireCallback(uintptr(p1))
			if !ok { return 0 }
			defer releaseCallback(entry)
			defer func() { if value := recover(); value != nil { entry.record(value); result = 0 } }()
			callback := stored.(func(unsafe.Pointer) uint8)
			return uintptr(callback(p0))
		})
	})
}
...
var libraryEnvVars = []string{"ZIGO_CBENUM_LIBRARY_PATH", "ZIGO_LIBRARY_PATH"}
...
func loadCandidate(path string) error {
	...
	addrOnLocation, err := resolveSymbol(handle, "zg_on_location_purego_v2")
	if err != nil { return fail("zg_on_location_purego_v2", err) }
	addrOnStream, err := resolveSymbol(handle, "zg_on_stream_purego_v2")
	if err != nil { return fail("zg_on_stream_purego_v2", err) }
	addrOnView, err := resolveSymbol(handle, "zg_on_view_purego_v2")
	if err != nil { return fail("zg_on_view_purego_v2", err) }
	...
	purego.RegisterFunc(&next.fnOnLocation, addrOnLocation)
	purego.RegisterFunc(&next.fnOnStream, addrOnStream)
	purego.RegisterFunc(&next.fnOnView, addrOnView)
...
// OnLocation calls the generated purego ABI wrapper for zg_on_location_purego_v2.
func OnLocation(callbackCallback, callbackToken uintptr) {
	bindings().fnOnLocation(callbackCallback, callbackToken)
}

// OnStream calls the generated purego ABI wrapper for zg_on_stream_purego_v2.
func OnStream(callbackCallback, callbackToken uintptr) {
	bindings().fnOnStream(callbackCallback, callbackToken)
}

// OnView calls the generated purego ABI wrapper for zg_on_view_purego_v2.
func OnView(callbackCallback, callbackToken uintptr) {
	bindings().fnOnView(callbackCallback, callbackToken)
}
