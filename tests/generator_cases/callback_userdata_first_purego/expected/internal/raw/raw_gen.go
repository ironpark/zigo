...
type nativeBindings struct {
	...
	fnReduce func(uintptr, int32, uintptr) int32
	fnVisit func(uintptr, uintptr)
}
...
func ensureCallbackDispatchers() {
	callbackDispatchersOnce.Do(func() {
		...
		callbackPointers[1] = purego.NewCallback(func(p0 int32, p1 uint8, p2 uint) (result uintptr) {
			entry, stored, ok := acquireCallback(uintptr(p2))
			if !ok { return 0 }
			defer releaseCallback(entry)
			defer func() { if value := recover(); value != nil { entry.record(value); result = 0 } }()
			callback := stored.(func(int32, uint8))
			callback(p0, p1)
			return 0
		})
	})
...
func Reduce(acc int32, reducerCallback, reducerToken uintptr) int32 {
	result := bindings().fnReduce(reducerToken, acc, reducerCallback)
	return int32(result)
}

// Visit calls the generated purego ABI wrapper for zg_visit_purego_v2.
func Visit(visitorCallback, visitorToken uintptr) {
	bindings().fnVisit(visitorCallback, visitorToken)
}
