...
var DefaultLibraryName = map[string]string{"darwin": "libinput_zigo.dylib", "linux": "libinput_zigo.so", "windows": "input_zigo.dll"}[runtime.GOOS]
...
type nativeBindings struct {
	...
	fnKeyPrintable func(uint8) uint8
	fnKeyCodepoint func(uint8) uint32
	fnKeyCombine func(uint8, uint8) uint8
	fnModeEnabled func(int32) uint8
	fnFromAscii func(uint8) uint8
}
...
var libraryEnvVars = []string{"ZIGO_INPUT_LIBRARY_PATH", "ZIGO_LIBRARY_PATH"}
...
func loadCandidate(path string) error {
	...
	addrKeyPrintable, err := resolveSymbol(handle, "zg_key_printable")
	if err != nil { return fail("zg_key_printable", err) }
	addrKeyCodepoint, err := resolveSymbol(handle, "zg_key_codepoint")
	if err != nil { return fail("zg_key_codepoint", err) }
	addrKeyCombine, err := resolveSymbol(handle, "zg_key_combine")
	if err != nil { return fail("zg_key_combine", err) }
	addrModeEnabled, err := resolveSymbol(handle, "zg_mode_enabled")
	if err != nil { return fail("zg_mode_enabled", err) }
	addrFromAscii, err := resolveSymbol(handle, "zg_from_ascii")
	if err != nil { return fail("zg_from_ascii", err) }
	...
	purego.RegisterFunc(&next.fnKeyPrintable, addrKeyPrintable)
	purego.RegisterFunc(&next.fnKeyCodepoint, addrKeyCodepoint)
	purego.RegisterFunc(&next.fnKeyCombine, addrKeyCombine)
	purego.RegisterFunc(&next.fnModeEnabled, addrModeEnabled)
	purego.RegisterFunc(&next.fnFromAscii, addrFromAscii)
...
// KeyPrintable calls the generated purego ABI wrapper for zg_key_printable.
func KeyPrintable(self uint8) uint8 {
	result := bindings().fnKeyPrintable(self)
	return uint8(result)
}

// KeyCodepoint calls the generated purego ABI wrapper for zg_key_codepoint.
func KeyCodepoint(self uint8) uint32 {
	result := bindings().fnKeyCodepoint(self)
	return uint32(result)
}

// KeyCombine calls the generated purego ABI wrapper for zg_key_combine.
func KeyCombine(self uint8, other uint8) uint8 {
	result := bindings().fnKeyCombine(self, other)
	return uint8(result)
}

// ModeEnabled calls the generated purego ABI wrapper for zg_mode_enabled.
func ModeEnabled(self int32) uint8 {
	result := bindings().fnModeEnabled(self)
	return uint8(result)
}

// FromAscii calls the generated purego ABI wrapper for zg_from_ascii.
func FromAscii(ch uint8) uint8 {
	result := bindings().fnFromAscii(ch)
	return uint8(result)
}
