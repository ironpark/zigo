...
var DefaultLibraryName = map[string]string{"darwin": "liboptional_return_checks_zigo.dylib", "linux": "liboptional_return_checks_zigo.so", "windows": "optional_return_checks_zigo.dll"}[runtime.GOOS]
...
var libraryEnvVars = []string{"ZIGO_OPTIONAL_RETURN_CHECKS_LIBRARY_PATH", "ZIGO_LIBRARY_PATH"}
...
func loadCandidate(path string) error {
	...
	if err != nil { return fail("zg_checked_level", err) }
	addrTerminalValue, err := resolveSymbol(handle, "zg_terminal_value")
...
// TerminalPassword calls the generated purego ABI wrapper for zg_terminal_password.
func TerminalPassword(self unsafe.Pointer) (int32, bool, int32) {
	var outResultHas uint8
	var outResult int32
	code := bindings().fnTerminalPassword(self, &outResultHas, &outResult)
	return outResult, outResultHas != 0, code
}

// CheckedLevel calls the generated purego ABI wrapper for zg_checked_level.
func CheckedLevel(level uint8) (int32, bool, int32) {
	var outResultHas uint8
	var outResult int32
	code := bindings().fnCheckedLevel(level, &outResultHas, &outResult)
	return outResult, outResultHas != 0, code
}

// TerminalValue calls the generated purego ABI wrapper for zg_terminal_value.
func TerminalValue(terminal unsafe.Pointer) (int32, bool, int32) {
	var outResultHas uint8
	var outResult int32
	code := bindings().fnTerminalValue(terminal, &outResultHas, &outResult)
	return outResult, outResultHas != 0, code
}
