...
#cgo LDFLAGS: ${SRCDIR}/../../../zig-out/lib/libimpl_zigo.a
#include "zigo_impl.h"
*/
import "C"
import (
...
// BufferPush calls the generated C ABI wrapper for zg_buffer_push.
...
func BufferDrain(self unsafe.Pointer, dst []uint8) (uint, int32) {
	...
	var outResult C.size_t
	code := int32(C.zg_buffer_drain((*C.zg_buffer)(self), dstPtr, C.size_t(len(dst)), &outResult))
	return uint(outResult), code
}
// StreamDump calls the generated C ABI wrapper for zg_stream_dump.
func StreamDump(self unsafe.Pointer, wHandle uintptr) int32 {
	code := int32(C.zg_stream_dump((*C.zg_stream)(self), C.size_t(wHandle)))
	return code
}
// StreamLoad calls the generated C ABI wrapper for zg_stream_load.
func StreamLoad(self unsafe.Pointer, rHandle uintptr, rData []byte) (uint, int32) {
	...
	var outResult C.size_t
	code := int32(C.zg_stream_load((*C.zg_stream)(self), rDataPtr, C.size_t(len(rData)), C.size_t(rHandle), &outResult))
	return uint(outResult), code
}
// BufferRender calls the generated C ABI wrapper for zg_buffer_render.
func BufferRender(self unsafe.Pointer, wHandle uintptr) (uint32, int32) {
	var outResult C.uint32_t
	code := int32(C.zg_buffer_render((*C.zg_buffer)(self), C.size_t(wHandle), &outResult))
	return uint32(outResult), code
}
// BufferFill calls the generated C ABI wrapper for zg_buffer_fill.
func BufferFill(self unsafe.Pointer, rHandle uintptr, rData []byte) int32 {
	var rDataPtr *C.uint8_t
	if rData != nil {
		if len(rData) != 0 {
			rDataPtr = (*C.uint8_t)(unsafe.Pointer(&rData[0]))
	...
	code := int32(C.zg_buffer_fill((*C.zg_buffer)(self), rDataPtr, C.size_t(len(rData)), C.size_t(rHandle)))
	return code
}
