...
type nativeBindings struct {
	...
	fnFilter func(int32, uintptr, uintptr) uint8
	fnEach func(uintptr, uintptr)
}
...
func ensureCallbackDispatchers() {
	callbackDispatchersOnce.Do(func() {
		callbackPointers[0] = purego.NewCallback(func(p0 int32, p1 uint8, p2 uint) (result uintptr) {
		...
		callbackPointers[1] = purego.NewCallback(func(p0 uint8, p1 uint) (result uintptr) {
			entry, stored, ok := acquireCallback(uintptr(p1))
			if !ok { return 0 }
			defer releaseCallback(entry)
			defer func() { if value := recover(); value != nil { entry.record(value); result = 0 } }()
			callback := stored.(func(uint8))
			callback(p0)
			return 0
		})
	})
...
func loadCandidate(path string) error {
	...
	if err != nil { return fail("zg_filter_purego_v2", err) }
	addrEach, err := resolveSymbol(handle, "zg_each_purego_v2")
	if err != nil { return fail("zg_each_purego_v2", err) }
	...
	purego.RegisterFunc(&next.fnEach, addrEach)
...
func Filter(value int32, predicateCallback, predicateToken uintptr) uint8 {
	result := bindings().fnFilter(value, predicateCallback, predicateToken)
	return uint8(result)
}

// Each calls the generated purego ABI wrapper for zg_each_purego_v2.
func Each(visitorCallback, visitorToken uintptr) {
	bindings().fnEach(visitorCallback, visitorToken)
}
