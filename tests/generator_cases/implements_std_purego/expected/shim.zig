...
export fn zg_stream_feed_impl(self: *target.Stream, bytes_ptr: [*c]const u8, bytes_len: usize) i32 {
    target.Stream.feed(self, if (bytes_len == 0) &.{} else bytes_ptr[0..bytes_len]) catch |err| return switch (err) {
        error.OutOfMemory => 1,
    };
    return 0;
}
export fn zg_buffer_push_impl(self: *target.Buffer, bytes_ptr: [*c]const u8, bytes_len: usize, out_result: *usize) i32 {
    const result = target.Buffer.push(self, if (bytes_len == 0) &.{} else bytes_ptr[0..bytes_len]);
    out_result.* = result;
    return 0;
}
...
export fn zg_stream_dump_purego_v2_impl(self: *target.Stream, w_fn: *const fn ([*c]const u8, usize, usize) callconv(.c) i32, w_userdata: usize) i32 {
    ...
    target.Stream.dump(self, &w_stream.interface) catch |err| return switch (err) {
        error.WriteFailed => 2,
    };
    return 0;
}
export fn zg_stream_load_purego_v2_impl(self: *target.Stream, r_fn: *const fn ([*c]u8, usize, usize) callconv(.c) i32, r_data: [*c]const u8, r_data_len: usize, r_userdata: usize, out_result: *usize) i32 {
    var r_stream_buffer: [65536]u8 = undefined;
    ...
    const result = target.Stream.load(self, r_stream_interface) catch |err| return switch (err) {
        error.ReadFailed => 3,
    };
    out_result.* = result;
    return 0;
}
export fn zg_buffer_render_purego_v2_impl(self: *target.Buffer, w_fn: *const fn ([*c]const u8, usize, usize) callconv(.c) i32, w_userdata: usize, out_result: *u32) i32 {
    var w_stream_buffer: [65536]u8 = undefined;
    var w_stream: ZigoWriterAdapter = .init(&w_stream_buffer, @ptrCast(w_fn), w_userdata);
    defer w_stream.interface.flush() catch {};
    const result = target.Buffer.render(self, &w_stream.interface);
    out_result.* = result;
    return 0;
}
export fn zg_buffer_fill_purego_v2_impl(self: *target.Buffer, r_fn: *const fn ([*c]u8, usize, usize) callconv(.c) i32, r_data: [*c]const u8, r_data_len: usize, r_userdata: usize) i32 {
    var r_stream_buffer: [65536]u8 = undefined;
    var r_stream: ZigoReaderAdapter = .init(&r_stream_buffer, @ptrCast(r_fn), r_userdata);
    var r_stream_fixed: std.Io.Reader = if (r_data != null) .fixed(r_data[0..r_data_len]) else undefined;
    const r_stream_interface: *std.Io.Reader = if (r_data != null) &r_stream_fixed else &r_stream.interface;
    target.Buffer.fill(self, r_stream_interface);
    return 0;
}
