...
var DefaultLibraryName = map[string]string{"darwin": "libimpl_zigo.dylib", "linux": "libimpl_zigo.so", "windows": "impl_zigo.dll"}[runtime.GOOS]
...
type nativeBindings struct {
	...
	fnBufferRender func(unsafe.Pointer, uintptr, uintptr, *uint32) int32
	fnBufferFill func(unsafe.Pointer, uintptr, unsafe.Pointer, uintptr, uintptr) int32
}
...
var libraryEnvVars = []string{"ZIGO_IMPL_LIBRARY_PATH", "ZIGO_LIBRARY_PATH"}
...
func loadCandidate(path string) error {
	...
	addrStreamFeed, err := resolveSymbol(handle, "zg_stream_feed")
	...
	if err != nil { return fail("zg_stream_dump_purego_v2", err) }
	addrStreamLoad, err := resolveSymbol(handle, "zg_stream_load_purego_v2")
	if err != nil { return fail("zg_stream_load_purego_v2", err) }
	addrBufferRender, err := resolveSymbol(handle, "zg_buffer_render_purego_v2")
	if err != nil { return fail("zg_buffer_render_purego_v2", err) }
	addrBufferFill, err := resolveSymbol(handle, "zg_buffer_fill_purego_v2")
	if err != nil { return fail("zg_buffer_fill_purego_v2", err) }
	...
	purego.RegisterFunc(&next.fnBufferRender, addrBufferRender)
	purego.RegisterFunc(&next.fnBufferFill, addrBufferFill)
...
// StreamFeed calls the generated purego ABI wrapper for zg_stream_feed.
func StreamFeed(self unsafe.Pointer, bytes []uint8) int32 {
	...
	code := bindings().fnStreamFeed(self, bytesPtr, uintptr(len(bytes)))
	return code
}

// BufferPush calls the generated purego ABI wrapper for zg_buffer_push.
func BufferPush(self unsafe.Pointer, bytes []uint8) (uint, int32) {
	var bytesPtr unsafe.Pointer
	if len(bytes) != 0 { bytesPtr = unsafe.Pointer(&bytes[0]) }
...
// BufferDrain calls the generated purego ABI wrapper for zg_buffer_drain.
func BufferDrain(self unsafe.Pointer, dst []uint8) (uint, int32) {
	var dstPtr unsafe.Pointer
	if len(dst) != 0 { dstPtr = unsafe.Pointer(&dst[0]) }
	var outResult uintptr
	code := bindings().fnBufferDrain(self, dstPtr, uintptr(len(dst)), &outResult)
	return uint(outResult), code
}

// StreamDump calls the generated purego ABI wrapper for zg_stream_dump_purego_v2.
func StreamDump(self unsafe.Pointer, wCallback, wHandle uintptr) int32 {
	code := bindings().fnStreamDump(self, wCallback, wHandle)
	return code
}

// StreamLoad calls the generated purego ABI wrapper for zg_stream_load_purego_v2.
func StreamLoad(self unsafe.Pointer, rCallback, rHandle uintptr, rData []byte) (uint, int32) {
	...
	var outResult uintptr
	code := bindings().fnStreamLoad(self, rCallback, rDataPtr, uintptr(len(rData)), rHandle, &outResult)
	runtime.KeepAlive(rData)
	return uint(outResult), code
}

// BufferRender calls the generated purego ABI wrapper for zg_buffer_render_purego_v2.
func BufferRender(self unsafe.Pointer, wCallback, wHandle uintptr) (uint32, int32) {
	var outResult uint32
	code := bindings().fnBufferRender(self, wCallback, wHandle, &outResult)
	return outResult, code
}

// BufferFill calls the generated purego ABI wrapper for zg_buffer_fill_purego_v2.
func BufferFill(self unsafe.Pointer, rCallback, rHandle uintptr, rData []byte) int32 {
	var rDataPtr unsafe.Pointer
	if rData != nil {
		if len(rData) != 0 {
			rDataPtr = unsafe.Pointer(&rData[0])
	...
	code := bindings().fnBufferFill(self, rCallback, rDataPtr, uintptr(len(rData)), rHandle)
	runtime.KeepAlive(rData)
	return code
}
