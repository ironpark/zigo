...
var DefaultLibraryName = map[string]string{"darwin": "libjob_zigo.dylib", "linux": "libjob_zigo.so", "windows": "job_zigo.dll"}[runtime.GOOS]
...
type nativeBindings struct {
	...
	fnJobTotal func(unsafe.Pointer, *float64) int32
}
...
var libraryEnvVars = []string{"ZIGO_JOB_LIBRARY_PATH", "ZIGO_LIBRARY_PATH"}
...
func loadCandidate(path string) error {
	...
	addrJobTotal, err := resolveSymbol(handle, "zg_job_total")
	if err != nil { return fail("zg_job_total", err) }
	...
	purego.RegisterFunc(&next.fnJobTotal, addrJobTotal)
...
// JobTotal calls the generated purego ABI wrapper for zg_job_total.
func JobTotal(self unsafe.Pointer) (float64, int32) {
	var outResult float64
	code := bindings().fnJobTotal(self, &outResult)
	return outResult, code
}
