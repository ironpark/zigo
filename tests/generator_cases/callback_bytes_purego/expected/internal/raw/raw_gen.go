...
var DefaultLibraryName = map[string]string{"darwin": "libcbbytes_zigo.dylib", "linux": "libcbbytes_zigo.so", "windows": "cbbytes_zigo.dll"}[runtime.GOOS]
...
type nativeBindings struct {
	...
	fnOnLog func(uintptr, uintptr)
	fnOnChunk func(uintptr, uintptr)
}
...
func ensureCallbackDispatchers() {
	callbackDispatchersOnce.Do(func() {
		callbackPointers[0] = purego.NewCallback(func(p0 unsafe.Pointer, p1 unsafe.Pointer, p1_len uint, p2 int32, p3 uint) (result uintptr) {
			...
			callback := stored.(func(string, string, int32))
			callback(zigoCStringString(p0), string(unsafe.Slice((*byte)(p1), p1_len)), p2)
			return 0
		})
		callbackPointers[1] = purego.NewCallback(func(p0 unsafe.Pointer, p0_len uint, p1 uint8, p2 uint) (result uintptr) {
			entry, stored, ok := acquireCallback(uintptr(p2))
			if !ok { return 0 }
			defer releaseCallback(entry)
			defer func() { if value := recover(); value != nil { entry.record(value); result = 0 } }()
			callback := stored.(func([]byte, uint8) uint8)
			return uintptr(callback(append([]byte(nil), unsafe.Slice((*byte)(p0), p0_len)...), p1))
		})
	})
}
...
var libraryEnvVars = []string{"ZIGO_CBBYTES_LIBRARY_PATH", "ZIGO_LIBRARY_PATH"}
...
func loadCandidate(path string) error {
	...
	addrOnLog, err := resolveSymbol(handle, "zg_on_log_purego_v2")
	if err != nil { return fail("zg_on_log_purego_v2", err) }
	addrOnChunk, err := resolveSymbol(handle, "zg_on_chunk_purego_v2")
	if err != nil { return fail("zg_on_chunk_purego_v2", err) }
	...
	purego.RegisterFunc(&next.fnOnLog, addrOnLog)
	purego.RegisterFunc(&next.fnOnChunk, addrOnChunk)
...
// OnLog calls the generated purego ABI wrapper for zg_on_log_purego_v2.
func OnLog(callbackCallback, callbackToken uintptr) {
	bindings().fnOnLog(callbackCallback, callbackToken)
}

// OnChunk calls the generated purego ABI wrapper for zg_on_chunk_purego_v2.
func OnChunk(callbackCallback, callbackToken uintptr) {
	bindings().fnOnChunk(callbackCallback, callbackToken)
}
