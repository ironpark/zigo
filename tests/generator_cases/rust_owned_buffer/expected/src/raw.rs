...
extern "C" {
    pub fn zg_take_digits(count: u32, out_result_ptr: *mut *const i32, out_result_len: *mut usize);
    pub fn zg_free_digits(values_ptr: *const i32, values_len: usize);
    pub fn zg_take_name(out_result_ptr: *mut *const u8, out_result_len: *mut usize);
    pub fn zg_free_name(value_ptr: *const u8, value_len: usize);
...
/// Calls the generated C ABI wrapper for `zg_take_digits`.
pub fn take_digits(count: u32) -> (*const i32, usize) {
    let mut out_result_ptr: *const i32 = core::ptr::null();
    let mut out_result_len: usize = 0;
    unsafe { zg_take_digits(count, &mut out_result_ptr, &mut out_result_len) };
    (out_result_ptr, out_result_len)
}

/// Calls the generated C ABI wrapper for `zg_free_digits`.
pub fn free_digits(values: &[i32]) {
    unsafe { zg_free_digits(values.as_ptr(), values.len()) }
}

/// Calls the generated C ABI wrapper for `zg_take_name`.
pub fn take_name() -> (*const u8, usize) {
    let mut out_result_ptr: *const u8 = core::ptr::null();
    let mut out_result_len: usize = 0;
    unsafe { zg_take_name(&mut out_result_ptr, &mut out_result_len) };
    (out_result_ptr, out_result_len)
}

/// Calls the generated C ABI wrapper for `zg_free_name`.
pub fn free_name(value: &str) {
    unsafe { zg_free_name(value.as_ptr(), value.len()) }
}
