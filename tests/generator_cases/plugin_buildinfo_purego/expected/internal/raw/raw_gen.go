...
type nativeBindings struct {
	...
	fnBuildinfoBuildInfo func() unsafe.Pointer
}
...
func loadCandidate(path string) error {
	...
	addrBuildinfoBuildInfo, err := resolveSymbol(handle, "zg_buildinfo_build_info")
	if err != nil { return fail("zg_buildinfo_build_info", err) }
	...
	purego.RegisterFunc(&next.fnBuildinfoBuildInfo, addrBuildinfoBuildInfo)
...
// BuildinfoBuildInfo calls the generated purego ABI wrapper for zg_buildinfo_build_info.
func BuildinfoBuildInfo() string {
	result := bindings().fnBuildinfoBuildInfo()
