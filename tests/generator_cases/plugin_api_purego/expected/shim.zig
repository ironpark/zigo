...
export fn zg_reduce_purego_v2_impl(ctx: usize, acc: i32, reducer: *const fn (i32, i32, usize) callconv(.c) i32, out_result: *i32) i32 {
    zg_reduce_purego_v2_bits_target_reducer.store(reducer, .release);
    const result = target.reduce(ctx, acc, &zg_reduce_purego_v2_bits_thunk_reducer) catch |err| return switch (err) {
        error.Failure => 1,
    };
    out_result.* = result;
    return 0;
