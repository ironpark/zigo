...
export fn zg_inspect_config_impl(config: *const target.Config, out_result: *target.Config) void {
    out_result.* = target.inspectConfig(config.*);
}
...
comptime {
    zigoAbiGuard("@sizeOf(Config)", 8, @sizeOf(target.Config));
    zigoAbiGuard("@alignOf(Config)", 4, @alignOf(target.Config));
    zigoAbiGuard("@offsetOf(Config, \"mode\")", 0, @offsetOf(target.Config, "mode"));
    zigoAbiGuard("@offsetOf(Config, \"y\")", 4, @offsetOf(target.Config, "y"));
}
