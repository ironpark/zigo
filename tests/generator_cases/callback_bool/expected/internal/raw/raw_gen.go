...
//export zg_each_go_callback_visitor
func zg_each_go_callback_visitor(p0 C.uint8_t, p1 C.size_t) {
	...
	callback := state.Fn.(func(uint8))
	callback(uint8(p0))
}

// Filter calls the generated C ABI wrapper for zg_filter.
func Filter(value int32, predicateHandle uintptr) uint8 {
	return uint8(C.zg_filter(C.int32_t(value), C.size_t(predicateHandle)))
}
// Each calls the generated C ABI wrapper for zg_each.
func Each(visitorHandle uintptr) {
	C.zg_each(C.size_t(visitorHandle))
}
