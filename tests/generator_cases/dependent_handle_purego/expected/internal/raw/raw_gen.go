...
var DefaultLibraryName = map[string]string{"darwin": "libdependent_handle_zigo.dylib", "linux": "libdependent_handle_zigo.so", "windows": "dependent_handle_zigo.dll"}[runtime.GOOS]
...
var libraryEnvVars = []string{"ZIGO_DEPENDENT_HANDLE_LIBRARY_PATH", "ZIGO_LIBRARY_PATH"}
...
func loadCandidate(path string) error {
	...
	if err != nil { return fail("zg_child_touch", err) }
...
// ChildFreeChild calls the generated purego ABI wrapper for zg_child_free_child.
func ChildFreeChild(self unsafe.Pointer) int32 {
	code := bindings().fnChildFreeChild(self)
	return code
}

// ChildTouch calls the generated purego ABI wrapper for zg_child_touch.
func ChildTouch(self unsafe.Pointer) int32 {
	code := bindings().fnChildTouch(self)
	return code
}
