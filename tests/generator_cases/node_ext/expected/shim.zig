...
export fn zg_counter_bump_impl(self: *target.Counter, by: i32, out_result: *target.Point) i32 {
    const result = target.Counter.bump(self, by);
    out_result.* = result;
    return 0;
}
export fn zg_counter_mode_impl(self: *target.Counter, out_result: *u8) i32 {
    const result = target.Counter.mode(self);
