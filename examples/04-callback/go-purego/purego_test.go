...
func TestBorrowedAndRetainedCallbacks(t *testing.T) {
	...
	// Observer, VoidObserver, Visitor, Predicate, Reducer, Logger, ByteSink,
	// and Inspector: eight unique ABI signatures.
	if got := zigoCallbackDispatcherCount(); got != 8 {
		t.Fatalf("permanent callback dispatchers = %d, want eight unique signatures", got)
	}
}
