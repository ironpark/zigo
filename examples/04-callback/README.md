...
5. `go/bytes_test.go`: `[*:0]const u8`·`[*]const u8`+`usize`를 `string`·`[]byte`로 받는 콜백
6. `go/handle_enum_test.go`: handle 포인터·enum·`bool`을 한 시그니처에서 받는 콜백
7. `go/cancel_test.go`, `go/lifecycle_test.go`: 취소와 수명 경계
