...
func newDocumentWithLines(t *testing.T, lines []string) *Document {
	...
	for _, line := range lines {
		if _, err := document.Write([]byte(line)); err != nil {
...
func TestDumpAndLoadRoundTrip(t *testing.T) {
	...
	if got := buffer.String(); got != "alpha\nbeta\ngamma\n" {
		t.Fatalf("WriteTo wrote %q", got)
	}

	restored := newDocumentWithLines(t, nil)
	read, err := restored.ReadFrom(bytes.NewReader(buffer.Bytes()))
	if err != nil {
		t.Fatalf("ReadFrom: %v", err)
	}
	if read != int64(buffer.Len()) {
		t.Fatalf("ReadFrom consumed %d bytes, wrote %d", read, buffer.Len())
	}
...
func TestDumpCrossesOncePerBuffer(t *testing.T) {
	...
	for i := 0; i < lines; i++ {
		if _, err := document.Write(line); err != nil {
			t.Fatalf("Write: %v", err)
		}
	}

	writer := &countingWriter{}
	if _, err := document.WriteTo(writer); err != nil {
		t.Fatalf("WriteTo: %v", err)
	}

	total := lines * lineBytes
	if writer.sink.Len() != total {
		t.Fatalf("WriteTo wrote %d bytes, want %d", writer.sink.Len(), total)
	}
	ceiling := (total + bufferBytes - 1) / bufferBytes
	if len(writer.writes) > ceiling {
		t.Fatalf("WriteTo cost %d Write calls for %d bytes; at most %d expected", len(writer.writes), total, ceiling)
	}
	...
	if len(writer.writes) < 2 {
		t.Fatalf("WriteTo cost %d Write calls; the payload is larger than the buffer", len(writer.writes))
	}
}
...
func TestWriterErrorIsReturnedAndIdentifiable(t *testing.T) {
	...
	_, err := document.WriteTo(&failingWriter{err: sentinel})
	if err == nil {
		t.Fatal("WriteTo returned no error for a failing writer")
	}
	if !errors.Is(err, sentinel) {
		t.Fatalf("WriteTo error %v does not wrap the writer's own error", err)
	}
	var streamErr *StreamError
	if !errors.As(err, &streamErr) {
		t.Fatalf("WriteTo error %v is not a *StreamError", err)
	}
...
func TestWriterPanicIsRethrownAsCallbackPanic(t *testing.T) {
	...
	_, _ = document.WriteTo(panickingWriter{})
}
...
func TestNilStreamIsAnArgumentError(t *testing.T) {
	...
	if !errors.Is(err, ErrNilStream) {
		t.Fatalf("WriteTo(nil) returned %v, want ErrNilStream", err)
	}
	if _, err := document.ReadFrom(nil); !errors.Is(err, ErrNilStream) {
		t.Fatalf("ReadFrom(nil) returned %v, want ErrNilStream", err)
	}
}
...
func TestReaderEndOfStreamStopsTheLoad(t *testing.T) {
	...
	read, err := document.ReadFrom(io.LimitReader(source, int64(len("alpha\nbeta\n"))))
	if err != nil {
		t.Fatalf("ReadFrom: %v", err)
	}
	if read != int64(len("alpha\nbeta\n")) {
		t.Fatalf("ReadFrom consumed %d bytes", read)
	}
	...
	if count != 2 {
		t.Fatalf("ReadFrom read %d lines, want 2", count)
	}
}
...
func TestReaderErrorIsReturnedAndIdentifiable(t *testing.T) {
	...
	_, err := document.ReadFrom(failingReader{err: sentinel})
	if !errors.Is(err, sentinel) {
		t.Fatalf("ReadFrom error %v does not wrap the reader's own error", err)
	}
}
...
func TestBytesReaderCostsNoCallback(t *testing.T) {
	...
	document := newDocumentWithLines(t, nil)
	read, err := document.ReadFrom(source)
	if err != nil {
		t.Fatalf("ReadFrom: %v", err)
	}
	if read != int64(len("alpha\nbeta\ngamma\n")) {
	...
	if source.reads != 0 {
		t.Fatalf("ReadFrom cost %d Read calls; a bytes.Buffer must cost none", source.reads)
	}
	count, err := document.Count()
	if err != nil {
		t.Fatalf("Count: %v", err)
	...
	if count != 3 {
		t.Fatalf("ReadFrom read %d lines, want 3", count)
	}
}
...
func TestNonBytesReaderStillCrossesForItsInput(t *testing.T) {
	...
	document := newDocumentWithLines(t, nil)
	if _, err := document.ReadFrom(source); err != nil {
		t.Fatalf("ReadFrom: %v", err)
	}
	if source.reads == 0 {
...
func TestEmptyBytesReaderCostsNoCallback(t *testing.T) {
	...
	if read != 0 || source.reads != 0 {
		t.Fatalf("ReadFrom consumed %d bytes over %d Read calls", read, source.reads)
	}
}
...
func TestZigoBytesHookOptsIntoTheFastPath(t *testing.T) {
	...
	if read != int64(len(source.data)) || source.reads != 0 {
		t.Fatalf("ReadFrom consumed %d bytes over %d Read calls", read, source.reads)
	}
}
