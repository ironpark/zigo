...
// The locker accessor closed that gap. Like the rest of this tree it runs from
// TestPuregoTaggedUnion once the library is loaded; CGO_ENABLED=0 rules out
// -race here, so the cgo tree runs the same scenario under the detector.
func assertConcurrentProjectionsRacingClose(t *testing.T) {
	t.Helper()
