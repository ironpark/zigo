// Code generated by zigo. DO NOT EDIT.

package tagged_union

import (
	"runtime"
	"sync"
	"unsafe"

	"example.com/zigo/tagged-union/internal/raw"
)

// Child is a caller-owned native handle. Call Close when it is no longer needed.
type Child struct {
	ptr     unsafe.Pointer
	mu      sync.Mutex
	active  int
	closed  bool
	poison  *NativePanicError
	cleanup runtime.Cleanup
}

// ChildRef is a borrowed Child reference that remains valid only while its parent is open.
type ChildRef struct {
	ptr    unsafe.Pointer
	parent zigoHandle
}

// zigoAcquire pins c open for one native call and hands back its pointer;
// the call ends with zigoRelease. A nil, closed, or poisoned handle is the error.
func (c *Child) zigoAcquire(operation string) (unsafe.Pointer, error) {
	if c == nil {
		return nil, &HandleError{Operation: operation}
	}
	c.mu.Lock()
	defer c.mu.Unlock()
	if c.closed || c.ptr == nil {
		return nil, &HandleError{Operation: operation}
	}
	if c.poison != nil {
		return nil, c.poison.poisoned(operation)
	}
	c.active++
	return c.ptr, nil
}

func (c *Child) zigoRelease() {
	if c == nil {
		return
	}
	c.mu.Lock()
	c.active--
	state, release := c.zigoTakeLocked()
	c.mu.Unlock()
	if release {
		zigoCleanupChild(state)
	}
}

// zigoPoison marks c unusable: a Zig panic unwound through native frames
// without running their defers, so the state behind it is unknown.
func (c *Child) zigoPoison(cause *NativePanicError) {
	if c == nil {
		return
	}
	c.mu.Lock()
	defer c.mu.Unlock()
	if c.poison == nil {
		c.poison = cause
		c.cleanup.Stop()
	}
}

func (c *ChildRef) zigoAcquire(operation string) (unsafe.Pointer, error) {
	if c == nil || c.ptr == nil {
		return nil, &HandleError{Operation: operation}
	}
	if c.parent != nil {
		if _, err := c.parent.zigoAcquire(operation); err != nil {
			return nil, err
		}
	}
	return c.ptr, nil
}

func (c *ChildRef) zigoRelease() {
	if c != nil && c.parent != nil {
		c.parent.zigoRelease()
	}
}

func (c *ChildRef) zigoPoison(cause *NativePanicError) {
	if c != nil && c.parent != nil {
		c.parent.zigoPoison(cause)
	}
}

type zigoChildCleanupState struct {
	ptr unsafe.Pointer
}

func zigoNewChild(ptr unsafe.Pointer) *Child {
	value := &Child{ptr: ptr}
	state := zigoChildCleanupState{ptr: ptr}
	value.cleanup = runtime.AddCleanup(value, zigoCleanupChild, state)
	return value
}

func zigoCleanupChild(state zigoChildCleanupState) {
	if state.ptr != nil {
		raw.ChildDeinit(state.ptr)
	}
}

// Close releases the native Child resources. It is safe to call more than once.
// It returns *HandleInUseError while a call is still inside native; otherwise the error is nil.
// Close does not wait: a call still inside native keeps the resources until it
// returns, and every call made after Close fails with *HandleError.
func (c *Child) Close() error {
	if c == nil {
		return nil
	}
	c.mu.Lock()
	if c.closed {
		c.mu.Unlock()
		return nil
	}
	c.closed = true
	c.cleanup.Stop()
	state, release := c.zigoTakeLocked()
	c.mu.Unlock()
	if release {
		zigoCleanupChild(state)
	}
	runtime.KeepAlive(c)
	return nil
}

// zigoTakeLocked hands out what is left to release once c is closed and no
// call is inside native; mu must be held. A poisoned handle keeps its native
// object: releasing state a panic left half-changed could fault, so it leaks.
func (c *Child) zigoTakeLocked() (zigoChildCleanupState, bool) {
	if !c.closed || c.active != 0 || c.ptr == nil {
		return zigoChildCleanupState{}, false
	}
	state := zigoChildCleanupState{ptr: c.ptr}
	c.ptr = nil
	if c.poison != nil {
		state.ptr = nil
	}
	return state, true
}

// Value is a caller-owned native handle. Call Close when it is no longer needed.
type Value struct {
	ptr     unsafe.Pointer
	mu      sync.Mutex
	active  int
	closed  bool
	poison  *NativePanicError
	cleanup runtime.Cleanup
}

// ValueRef is a borrowed Value reference that remains valid only while its parent is open.
type ValueRef struct {
	ptr    unsafe.Pointer
	parent zigoHandle
}

// zigoAcquire pins v open for one native call and hands back its pointer;
// the call ends with zigoRelease. A nil, closed, or poisoned handle is the error.
func (v *Value) zigoAcquire(operation string) (unsafe.Pointer, error) {
	if v == nil {
		return nil, &HandleError{Operation: operation}
	}
	v.mu.Lock()
	defer v.mu.Unlock()
	if v.closed || v.ptr == nil {
		return nil, &HandleError{Operation: operation}
	}
	if v.poison != nil {
		return nil, v.poison.poisoned(operation)
	}
	v.active++
	return v.ptr, nil
}

func (v *Value) zigoRelease() {
	if v == nil {
		return
	}
	v.mu.Lock()
	v.active--
	state, release := v.zigoTakeLocked()
	v.mu.Unlock()
	if release {
		zigoCleanupValue(state)
	}
}

// zigoPoison marks v unusable: a Zig panic unwound through native frames
// without running their defers, so the state behind it is unknown.
func (v *Value) zigoPoison(cause *NativePanicError) {
	if v == nil {
		return
	}
	v.mu.Lock()
	defer v.mu.Unlock()
	if v.poison == nil {
		v.poison = cause
		v.cleanup.Stop()
	}
}

func (v *ValueRef) zigoAcquire(operation string) (unsafe.Pointer, error) {
	if v == nil || v.ptr == nil {
		return nil, &HandleError{Operation: operation}
	}
	if v.parent != nil {
		if _, err := v.parent.zigoAcquire(operation); err != nil {
			return nil, err
		}
	}
	return v.ptr, nil
}

func (v *ValueRef) zigoRelease() {
	if v != nil && v.parent != nil {
		v.parent.zigoRelease()
	}
}

func (v *ValueRef) zigoPoison(cause *NativePanicError) {
	if v != nil && v.parent != nil {
		v.parent.zigoPoison(cause)
	}
}

type zigoValueCleanupState struct {
	ptr unsafe.Pointer
}

func zigoNewValue(ptr unsafe.Pointer) *Value {
	value := &Value{ptr: ptr}
	state := zigoValueCleanupState{ptr: ptr}
	value.cleanup = runtime.AddCleanup(value, zigoCleanupValue, state)
	return value
}

func zigoCleanupValue(state zigoValueCleanupState) {
	if state.ptr != nil {
		raw.ValueDeinit(state.ptr)
	}
}

// Close releases the native Value resources. It is safe to call more than once.
// It returns *HandleInUseError while a call is still inside native; otherwise the error is nil.
// Close does not wait: a call still inside native keeps the resources until it
// returns, and every call made after Close fails with *HandleError.
func (v *Value) Close() error {
	if v == nil {
		return nil
	}
	v.mu.Lock()
	if v.closed {
		v.mu.Unlock()
		return nil
	}
	v.closed = true
	v.cleanup.Stop()
	state, release := v.zigoTakeLocked()
	v.mu.Unlock()
	if release {
		zigoCleanupValue(state)
	}
	runtime.KeepAlive(v)
	return nil
}

// zigoTakeLocked hands out what is left to release once v is closed and no
// call is inside native; mu must be held. A poisoned handle keeps its native
// object: releasing state a panic left half-changed could fault, so it leaks.
func (v *Value) zigoTakeLocked() (zigoValueCleanupState, bool) {
	if !v.closed || v.active != 0 || v.ptr == nil {
		return zigoValueCleanupState{}, false
	}
	state := zigoValueCleanupState{ptr: v.ptr}
	v.ptr = nil
	if v.poison != nil {
		state.ptr = nil
	}
	return state, true
}

// Signal is a caller-owned native handle. Call Close when it is no longer needed.
type Signal struct {
	ptr     unsafe.Pointer
	mu      sync.Mutex
	active  int
	closed  bool
	poison  *NativePanicError
	cleanup runtime.Cleanup
}

// zigoAcquire pins s open for one native call and hands back its pointer;
// the call ends with zigoRelease. A nil, closed, or poisoned handle is the error.
func (s *Signal) zigoAcquire(operation string) (unsafe.Pointer, error) {
	if s == nil {
		return nil, &HandleError{Operation: operation}
	}
	s.mu.Lock()
	defer s.mu.Unlock()
	if s.closed || s.ptr == nil {
		return nil, &HandleError{Operation: operation}
	}
	if s.poison != nil {
		return nil, s.poison.poisoned(operation)
	}
	s.active++
	return s.ptr, nil
}

func (s *Signal) zigoRelease() {
	if s == nil {
		return
	}
	s.mu.Lock()
	s.active--
	state, release := s.zigoTakeLocked()
	s.mu.Unlock()
	if release {
		zigoCleanupSignal(state)
	}
}

// zigoPoison marks s unusable: a Zig panic unwound through native frames
// without running their defers, so the state behind it is unknown.
func (s *Signal) zigoPoison(cause *NativePanicError) {
	if s == nil {
		return
	}
	s.mu.Lock()
	defer s.mu.Unlock()
	if s.poison == nil {
		s.poison = cause
		s.cleanup.Stop()
	}
}

type zigoSignalCleanupState struct {
	ptr unsafe.Pointer
}

func zigoNewSignal(ptr unsafe.Pointer) *Signal {
	value := &Signal{ptr: ptr}
	state := zigoSignalCleanupState{ptr: ptr}
	value.cleanup = runtime.AddCleanup(value, zigoCleanupSignal, state)
	return value
}

func zigoCleanupSignal(state zigoSignalCleanupState) {
	if state.ptr != nil {
		raw.SignalDeinit(state.ptr)
	}
}

// Close releases the native Signal resources. It is safe to call more than once.
// It returns *HandleInUseError while a call is still inside native; otherwise the error is nil.
// Close does not wait: a call still inside native keeps the resources until it
// returns, and every call made after Close fails with *HandleError.
func (s *Signal) Close() error {
	if s == nil {
		return nil
	}
	s.mu.Lock()
	if s.closed {
		s.mu.Unlock()
		return nil
	}
	s.closed = true
	s.cleanup.Stop()
	state, release := s.zigoTakeLocked()
	s.mu.Unlock()
	if release {
		zigoCleanupSignal(state)
	}
	runtime.KeepAlive(s)
	return nil
}

// zigoTakeLocked hands out what is left to release once s is closed and no
// call is inside native; mu must be held. A poisoned handle keeps its native
// object: releasing state a panic left half-changed could fault, so it leaks.
func (s *Signal) zigoTakeLocked() (zigoSignalCleanupState, bool) {
	if !s.closed || s.active != 0 || s.ptr == nil {
		return zigoSignalCleanupState{}, false
	}
	state := zigoSignalCleanupState{ptr: s.ptr}
	s.ptr = nil
	if s.poison != nil {
		state.ptr = nil
	}
	return state, true
}

// Palette is a caller-owned native handle. Call Close when it is no longer needed.
type Palette struct {
	ptr     unsafe.Pointer
	mu      sync.Mutex
	active  int
	closed  bool
	poison  *NativePanicError
	cleanup runtime.Cleanup
}

// zigoAcquire pins p open for one native call and hands back its pointer;
// the call ends with zigoRelease. A nil, closed, or poisoned handle is the error.
func (p *Palette) zigoAcquire(operation string) (unsafe.Pointer, error) {
	if p == nil {
		return nil, &HandleError{Operation: operation}
	}
	p.mu.Lock()
	defer p.mu.Unlock()
	if p.closed || p.ptr == nil {
		return nil, &HandleError{Operation: operation}
	}
	if p.poison != nil {
		return nil, p.poison.poisoned(operation)
	}
	p.active++
	return p.ptr, nil
}

func (p *Palette) zigoRelease() {
	if p == nil {
		return
	}
	p.mu.Lock()
	p.active--
	state, release := p.zigoTakeLocked()
	p.mu.Unlock()
	if release {
		zigoCleanupPalette(state)
	}
}

// zigoPoison marks p unusable: a Zig panic unwound through native frames
// without running their defers, so the state behind it is unknown.
func (p *Palette) zigoPoison(cause *NativePanicError) {
	if p == nil {
		return
	}
	p.mu.Lock()
	defer p.mu.Unlock()
	if p.poison == nil {
		p.poison = cause
		p.cleanup.Stop()
	}
}

type zigoPaletteCleanupState struct {
	ptr unsafe.Pointer
}

func zigoNewPalette(ptr unsafe.Pointer) *Palette {
	value := &Palette{ptr: ptr}
	state := zigoPaletteCleanupState{ptr: ptr}
	value.cleanup = runtime.AddCleanup(value, zigoCleanupPalette, state)
	return value
}

func zigoCleanupPalette(state zigoPaletteCleanupState) {
	if state.ptr != nil {
		raw.PaletteDeinit(state.ptr)
	}
}

// Close releases the native Palette resources. It is safe to call more than once.
// It returns *HandleInUseError while a call is still inside native; otherwise the error is nil.
// Close does not wait: a call still inside native keeps the resources until it
// returns, and every call made after Close fails with *HandleError.
func (p *Palette) Close() error {
	if p == nil {
		return nil
	}
	p.mu.Lock()
	if p.closed {
		p.mu.Unlock()
		return nil
	}
	p.closed = true
	p.cleanup.Stop()
	state, release := p.zigoTakeLocked()
	p.mu.Unlock()
	if release {
		zigoCleanupPalette(state)
	}
	runtime.KeepAlive(p)
	return nil
}

// zigoTakeLocked hands out what is left to release once p is closed and no
// call is inside native; mu must be held. A poisoned handle keeps its native
// object: releasing state a panic left half-changed could fault, so it leaks.
func (p *Palette) zigoTakeLocked() (zigoPaletteCleanupState, bool) {
	if !p.closed || p.active != 0 || p.ptr == nil {
		return zigoPaletteCleanupState{}, false
	}
	state := zigoPaletteCleanupState{ptr: p.ptr}
	p.ptr = nil
	if p.poison != nil {
		state.ptr = nil
	}
	return state, true
}
